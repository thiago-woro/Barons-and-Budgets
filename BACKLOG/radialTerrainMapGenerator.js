...
//this file is just an experiment, it draws a island but the cutoff is pretty harsh
     ...
     for (let y = 0; y < rows; y++) {
       ...
       for (let x = 0; x < rows; x++) {
         const distanceToCenter = Math.sqrt(
           Math.pow(x - rows / 2, 2) + Math.pow(y - rows / 2, 2)
         );
         const maxRadius = rows / 2;
         ...
         if (distanceToCenter < maxRadius) {
           ...
           // Set cells outside the island radius to water
           terrainMap[y][x] = WATER_SHADES[0];
           waterCells.push({ x, y, color: WATER_SHADES[0] });
         }
       }
     }
     console.log(
